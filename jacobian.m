...
for i=1:nbus
    ...
    if(i ~= sb)
        J1(i, i) = J1(i, i);
        J3(i, i) = J3(i, i);
        J2(i, i) = J22+(2*v(j)*cos(ang(i, i)));
        J4(i, i) = -J44-(2*v(j)*sin(ang(i, i)));
    end
end
for i=1:nbus
    for j=1:nbus
        if(i~=sb & i~=j & j~=sb)
            ...
            J2(i, j) = v(i)*v(j)*ym(i, j)*cos(ang(i, j)+delta(j)-delta(i));
            J4(i, j) = -v(i)*v(j)*ym(i, j)*sin(ang(i, j)+delta(j)-delta(i));
        end
    end
end
for i=1:nbus
